...
#    else:
    pt_2_1 = (points_marker_2[0] + points_marker_2[3])/2
    pt_2_2 = (points_marker_2[1] + points_marker_2[2])/2
    cv2.line(image, (pt_2_1[0], pt_2_1[1]), (pt_2_2[0], pt_2_2[1]), (0, 255, 0), 5)
    slope = ((pt_1_2[1] - pt_1_1[1])/(pt_1_2[0] - pt_1_1[0]))
    c1 = pt_1_1[1] - (pt_1_1[0] * slope)
    c2 = pt_2_1[1] - (pt_2_1[0] * ((pt_2_2[1] - pt_2_1[1])/(pt_2_2[0] - pt_2_1[0])))
    distance = abs(c2 - c1)/(np.sqrt(1+ (slope ** 2)))
...
def get_movement(obj, mar1_pts, mar2_pts, projection, image):
    global accn, glyph_width, glyph_height
    ...
    while(not is_halt):
        ...
        for face in obj.faces:
            ...
            points = np.array([[p[0] + glyph_width / 2, p[1] + glyph_height / 2, p[2]] for p in points])
...
def process_video(video_name):
    ...
    while(True):
        ...
        ret, frame = cap.read()
        ################## deleteThis ############
        frame = cv2.imread("test_case16.jpg")
        deleteThis = cv2.imread("test_case16.jpg")
#        deleteThis2 = cv2.imread("test_case3.jpg")
       ...
       # frame = cv2.undistort(frame, mtx, dist, None, new_camera_matrix)
        ...
        for contour in contours:
            deleteThis1 = cv2.imread("test_case16.jpg")
...
#            rotation_matrix = get_rotation_matrix_between_glyphs(glyph_match_1, glyph_match_2)
#            print("rotation matrix")
#            print(rotation_matrix)
#            glyph1_3d_transformation = np.dot(rotation_matrix, glyph1_3d_transformation)
#            print("new matrix")
#            print(glyph1_3d_transformation)
...
def object_initialization():
    ...
    object2 = OBJ("fox.obj", swapyz=True)
    object1 = OBJ("rat.obj", swapyz=True)
