...
def get_movement(obj, mar1_pts, mar2_pts, projection, image):
    global accn, glyph_width, glyph_height, glyph_match_1, glyph_match_2
    ...
    while(not is_halt):
        ...
        for face in obj.faces:
            ...
            dst = np.array([[p[0][0] + max_width / 2, p[0][1] + max_ht / 2] for p in dst])
...
def process_video(video_name):
    ...
    while(True):
        ...
        ret, frame = cap.read()
        ################## deleteThis ############
        frame = cv2.imread("test_case17.jpg")
        deleteThis = cv2.imread("test_case17.jpg")
...
#        #print(len(contours))
        for contour in contours:
            deleteThis1 = cv2.imread("test_case17.jpg")
